...
namespace Archetypical.Software.SchemaRegistry.Controllers
{
    ...
    public class RegistrationController : ControllerBase
    {
        ...
        public virtual async Task<IActionResult> CreateSchema([FromRoute][Required] Guid groupId, [FromRoute][Required] Guid schemaId)
        {
            ...
            var gr = _context.Schemas.FirstOrDefault(x => x.Id == groupId);
            if (gr == null)
                return NotFound();

            var schemas = _context.Versions.Where(x => x.SchemaGroupId == groupId).ToList()
            ...
            if (schema.Hash != latest.Hash)
            {
                ...
                await _context.Versions.AddAsync(schema);
